...
public: //implementation of VehicleSimApiBase
    ...
    // Vehicle backends expose their API through the common PawnSimApi type so
    // SimMode factories do not need unsafe downcasts to a concrete backend.
...
protected: //additional interface for derived class
    ...
    virtual std::string getImageCaptureMetadata() const;
    virtual void onCollisionObserved(const CollisionInfo& collision_info);
...
private: //vars
    ...
    int collision_signal_id_ = -1;
    int pawn_tick_signal_id_ = -1;
