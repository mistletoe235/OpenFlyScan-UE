...
namespace airlib
{
        ...
        : RpcLibServerBase(api_provider, server_address, port)
    {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("getDjiHilStatus", [&](const std::string& vehicle_name) -> std::string {
            return getVehicleApi(vehicle_name)->getDjiHilStatus();
        });
        (static_cast<rpc::server*>(getServer()))->bind("getDjiHilCapabilities", [&](const std::string& vehicle_name) -> std::string {
            return getVehicleApi(vehicle_name)->getDjiHilCapabilities();
        });
        (static_cast<rpc::server*>(getServer()))->bind("getDjiHilTiming", [&](const std::string& vehicle_name) -> std::string {
            return getVehicleApi(vehicle_name)->getDjiHilTiming();
        });
        (static_cast<rpc::server*>(getServer()))->bind("getDjiHilDecodedState", [&](const std::string& vehicle_name) -> std::string {
            return getVehicleApi(vehicle_name)->getDjiHilDecodedState();
        });
        (static_cast<rpc::server*>(getServer()))->bind("sendDjiHilSafetyEvent", [&](uint32_t kind, double score, const std::string& reason, const std::string& vehicle_name) -> bool {
            return getVehicleApi(vehicle_name)->sendDjiHilSafetyEvent(kind, score, reason);
        });
    }
