...
namespace airlib
{
    ...
    public: //types
        ...
        static constexpr char const* kVehicleTypeDjiHil = "djihil";
        ...
        struct DjiHilSetting
        {
            std::string backend = "replay";
            std::string state_host = "127.0.0.1";
            int state_port = 14560;
            std::string control_host = "127.0.0.1";
            int control_port = 14561;
            int stale_timeout_ms = 200;
            int interpolation_delay_ms = 30;
            int max_extrapolation_ms = 20;
            std::string replay_file;
            bool replay_loop = false;
            std::string udp_bind_address = "0.0.0.0";
            int udp_port = 30020;
            int android_reply_port = 30021;
            std::string frame_tcp_bind_address = "0.0.0.0";
            int frame_tcp_port = 30022;
            int peer_timeout_ms = 1000;
            int heartbeat_interval_ms = 250;
            int frame_width = 1440;
            int frame_height = 1080;
            std::string frame_format = "jpeg";
            int jpeg_quality = 92;
            int maximum_frame_bytes = 16 * 1024 * 1024;
            std::string frame_camera_name = "front_custom";
            int maximum_frame_rate = 30;
        };
        ...
        struct VehicleSetting
        {
            ...
            RCSettings rc;
            std::shared_ptr<DjiHilSetting> dji_hil;
    ...
    public: //methods
        ...
        const VehicleSetting* getVehicleSetting(const std::string& vehicle_name) const
        {
            ...
            // A legacy map can also contain a pre-existing pawn while the user
            // settings only name an auto-created vehicle (for example Drone1).
            // The old code dereferenced end() in that case and the resulting
            // invalid VehicleSetting pointer looked like an Eigen alignment
            // crash in camera setup on Linux.
            if (it == vehicles.end() && !vehicles.empty())
                it = vehicles.begin();
            return it != vehicles.end() ? it->second.get() : nullptr;
        }
    ...
    private:
                                                                    ...
                                                                    const CameraSetting& camera_defaults)
        {
            ...
            if (vehicle_type == kVehicleTypeDjiHil) {
                vehicle_setting->dji_hil = std::make_shared<DjiHilSetting>();
                Settings dji_hil_json;
                if (settings_json.getChild("DjiHil", dji_hil_json)) {
                    auto& dji_hil = *vehicle_setting->dji_hil;
                    dji_hil.backend = Utils::toLower(dji_hil_json.getString("Backend", dji_hil.backend));
                    dji_hil.state_host = dji_hil_json.getString("StateHost", dji_hil.state_host);
                    dji_hil.state_port = dji_hil_json.getInt("StatePort", dji_hil.state_port);
                    dji_hil.control_host = dji_hil_json.getString("ControlHost", dji_hil.control_host);
                    dji_hil.control_port = dji_hil_json.getInt("ControlPort", dji_hil.control_port);
                    dji_hil.stale_timeout_ms = dji_hil_json.getInt("StaleTimeoutMs", dji_hil.stale_timeout_ms);
                    dji_hil.interpolation_delay_ms = dji_hil_json.getInt("InterpolationDelayMs", dji_hil.interpolation_delay_ms);
                    dji_hil.max_extrapolation_ms = dji_hil_json.getInt("MaxExtrapolationMs", dji_hil.max_extrapolation_ms);
                    dji_hil.replay_file = dji_hil_json.getString("ReplayFile", dji_hil.replay_file);
                    dji_hil.replay_loop = dji_hil_json.getBool("ReplayLoop", dji_hil.replay_loop);
                    dji_hil.udp_bind_address = dji_hil_json.getString("UdpBindAddress", dji_hil.udp_bind_address);
                    dji_hil.udp_port = dji_hil_json.getInt("UdpPort", dji_hil.udp_port);
                    dji_hil.android_reply_port = dji_hil_json.getInt("AndroidReplyPort", dji_hil.android_reply_port);
                    dji_hil.frame_tcp_bind_address = dji_hil_json.getString("FrameTcpBindAddress", dji_hil.frame_tcp_bind_address);
                    dji_hil.frame_tcp_port = dji_hil_json.getInt("FrameTcpPort", dji_hil.frame_tcp_port);
                    dji_hil.peer_timeout_ms = dji_hil_json.getInt("PeerTimeoutMs", dji_hil.peer_timeout_ms);
                    dji_hil.heartbeat_interval_ms = dji_hil_json.getInt("HeartbeatIntervalMs", dji_hil.heartbeat_interval_ms);
                    dji_hil.frame_width = dji_hil_json.getInt("FrameWidth", dji_hil.frame_width);
                    dji_hil.frame_height = dji_hil_json.getInt("FrameHeight", dji_hil.frame_height);
                    dji_hil.frame_format = Utils::toLower(dji_hil_json.getString("FrameFormat", dji_hil.frame_format));
                    dji_hil.jpeg_quality = dji_hil_json.getInt("JpegQuality", dji_hil.jpeg_quality);
                    dji_hil.maximum_frame_bytes = dji_hil_json.getInt("MaximumFrameBytes", dji_hil.maximum_frame_bytes);
                    dji_hil.frame_camera_name = dji_hil_json.getString("FrameCameraName", dji_hil.frame_camera_name);
                    dji_hil.maximum_frame_rate = dji_hil_json.getInt("MaximumFrameRate", dji_hil.maximum_frame_rate);
                }
            }
