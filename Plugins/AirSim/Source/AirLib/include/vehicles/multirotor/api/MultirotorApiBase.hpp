...
namespace airlib
{
    ...
    public: //these APIs uses above low level APIs
        ...
        virtual MultirotorState getMultirotorState() const
        {
        ...
        // Optional observation-backend extensions. Existing vehicle types keep
        // the standard API unchanged and report that DJI HIL is unavailable.
        virtual std::string getDjiHilStatus() const
        {
            return "{\"available\":false}";
        }
        virtual std::string getDjiHilCapabilities() const
        {
            return "{\"available\":false}";
        }
        virtual std::string getDjiHilTiming() const
        {
            return "{\"available\":false}";
        }
        virtual std::string getDjiHilDecodedState() const
        {
            return "{\"available\":false}";
        }
        // Observation-side safety notification only. This must never be wired
        // to aircraft motion or flight-controller commands.
    ...
    public:
        virtual void setGoalToCurrentPosition(const YawMode& yaw_mode = YawMode::Zero())
        {
            SingleTaskCall lock(this);
            moveToPositionInternal(getPosition(), yaw_mode);
        }
