...
PawnSimApi::~PawnSimApi()
{
    if (params_.pawn_events != nullptr) {
        if (collision_signal_id_ >= 0)
            params_.pawn_events->getCollisionSignal().disconnect(collision_signal_id_);
        if (pawn_tick_signal_id_ >= 0)
            params_.pawn_events->getPawnTickSignal().disconnect(pawn_tick_signal_id_);
    }
}
...
void PawnSimApi::initialize()
{
    ...
    image_capture_.reset(new UnrealImageCapture(&cameras_, [this]() {
        return getImageCaptureMetadata();
    }));

    //add listener for pawn's collision event
    collision_signal_id_ = params_.pawn_events->getCollisionSignal().connect_member(this, &PawnSimApi::onCollision);
    pawn_tick_signal_id_ = params_.pawn_events->getPawnTickSignal().connect_member(this, &PawnSimApi::pawnTick);
}
...
void PawnSimApi::onCollisionObserved(const CollisionInfo& collision_info)
{
    unused(collision_info);
}

std::string PawnSimApi::getImageCaptureMetadata() const
{
    return std::string();
}
...
void PawnSimApi::setupCamerasFromSettings(const common_utils::UniqueValueMap<std::string, APIPCamera*>& cameras)
{
    ...
    for (const auto& p : cameras.getMap()) {
        // Some legacy AirSim pawn blueprints lose their ChildActor camera
        // instances when loaded in UE5/Chaos (the old AirSimAssets map still
        // references PhysXVehicles). Never retain those null aliases: the
        // setup loop below dereferences every value in the map.
        if (IsValid(p.second) && p.second->HasActorBegunPlay() && !p.second->IsActorBeingDestroyed())
            cameras_.insert_or_assign(p.first, p.second);
        else
            UE_LOG(LogTemp, Warning, TEXT("AirSim camera '%s' is not play-ready on pawn '%s'; skipping legacy camera"),
                   UTF8_TO_TCHAR(p.first.c_str()), *GetNameSafe(params_.pawn));
    }
...
void PawnSimApi::createCamerasFromSettings()
{
    ...
    if (!IsValid(params_.pawn) || !IsValid(bodyMesh) || params_.pip_camera_class == nullptr) {
        UE_LOG(LogTemp, Error, TEXT("AirSim cannot create configured cameras for pawn '%s': pawn/root/PIP camera class is invalid"),
               *GetNameSafe(params_.pawn));
        return;
    }
    ...
    for (const auto& camera_setting_pair : getVehicleSetting()->cameras) {
        ...
        if (!IsValid(camera)) {
            UE_LOG(LogTemp, Error, TEXT("AirSim failed to spawn configured camera '%s' for pawn '%s'"),
                   UTF8_TO_TCHAR(camera_setting_pair.first.c_str()), *GetNameSafe(params_.pawn));
            continue;
        }
                             ...
                             bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
    ...
    onCollisionObserved(state_.collision_info);
}
