...
void UAirBlueprintLib::CompressImageArray(int32 width, int32 height, const TArray<FColor>& src, TArray<uint8>& dest)
{
    ...
    // PNGs are RGBA while FColor storage is BGRA. Convert directly into the
    // thumbnail buffer instead of making a full-size mutable copy first.
    const int32 PixelCount = width * height;
    ThumbnailByteArray.SetNumUninitialized(PixelCount * sizeof(FColor));
    FColor* Converted = reinterpret_cast<FColor*>(ThumbnailByteArray.GetData());
    for (int32 Index = 0; Index < PixelCount; ++Index) {
        Converted[Index] = src[Index];
        Swap(Converted[Index].R, Converted[Index].B);
    }
