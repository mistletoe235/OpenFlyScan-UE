...
void APIPCamera::PostInitializeComponents()
{
    ...
    secondary_render_targets_.Init(nullptr, imageTypeCount());
...
void APIPCamera::BeginPlay()
{
    ...
    for (unsigned int image_type = 0; image_type < imageTypeCount(); ++image_type) {
        ...
        secondary_render_targets_[image_type] = NewObject<UTextureRenderTarget2D>(this);
        secondary_render_targets_[image_type]->AddToRoot();
    }
...
void APIPCamera::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    ...
    for (unsigned int image_type = 0; image_type < imageTypeCount(); ++image_type) {
        //use final color for all calculations
        if (secondary_render_targets_[image_type] != nullptr) {
            secondary_render_targets_[image_type]->RemoveFromRoot();
            secondary_render_targets_[image_type] = nullptr;
        }
...
void APIPCamera::setCaptureUpdate(USceneCaptureComponent2D* capture, bool nodisplay)
{
    ...
    capture->bAlwaysPersistRenderingState = !nodisplay;
}
    ...
    for (int image_type = -1; image_type < image_count; ++image_type) {
        ...
        if (image_type >= 0) { //scene capture components
            ...
            case ImageType::Infrared:
                ...
                updateCaptureComponentSetting(captures_[image_type], secondary_render_targets_[image_type], false, pixel_format, capture_setting, ned_transform, false);
                break;
            ...
            case ImageType::SurfaceNormals:
                ...
                updateCaptureComponentSetting(captures_[image_type], secondary_render_targets_[image_type], true, pixel_format, capture_setting, ned_transform, true);
                break;
            ...
            default:
                ...
                updateCaptureComponentSetting(captures_[image_type], secondary_render_targets_[image_type], true, pixel_format, capture_setting, ned_transform, false);
                break;
            }
...
{
    if (!IsValid(distortion_material_static_) || !IsValid(outer))
        return;
    if (distortion_materials_.Num() != static_cast<int32>(imageTypeCount() + 1))
        distortion_materials_.SetNumZeroed(imageTypeCount() + 1);

    UMaterialInstanceDynamic* distortion_material = UMaterialInstanceDynamic::Create(distortion_material_static_, outer);
    if (!IsValid(distortion_material))
        return;
...
{
    if (!settings.Enabled || !IsValid(noise_material_static_) || !IsValid(outer))
        return;
    if (noise_materials_.Num() != static_cast<int32>(imageTypeCount() + 1))
        noise_materials_.SetNumZeroed(imageTypeCount() + 1);

    UMaterialInstanceDynamic* noise_material = UMaterialInstanceDynamic::Create(noise_material_static_, outer);
    if (!IsValid(noise_material))
        return;
...
UTextureRenderTarget2D* APIPCamera::getSecondaryRenderTarget(
    const APIPCamera::ImageType type)
{
    return secondary_render_targets_[Utils::toNumeric(type)];
}
