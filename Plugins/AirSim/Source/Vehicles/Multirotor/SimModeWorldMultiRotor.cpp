...
bool ASimModeWorldMultiRotor::isVehicleTypeSupported(const std::string& vehicle_type) const
{
    return ((vehicle_type == AirSimSettings::kVehicleTypeSimpleFlight) ||
            ...
            (vehicle_type == AirSimSettings::kVehicleTypeDjiHil));
}
...
{
    const auto* VehicleSetting = getSettings().getVehicleSetting(pawn_sim_api_params.vehicle_name);
    std::unique_ptr<PawnSimApi> vehicle_sim_api;
    if (VehicleSetting != nullptr && VehicleSetting->vehicle_type == AirSimSettings::kVehicleTypeDjiHil)
        vehicle_sim_api = std::make_unique<DjiHilPawnSimApi>(pawn_sim_api_params);
    else
        vehicle_sim_api = std::make_unique<MultirotorPawnSimApi>(pawn_sim_api_params);
                                                                    ...
                                                                    const PawnSimApi* sim_api) const
{
    ...
    return sim_api != nullptr ? sim_api->getVehicleApiBase() : nullptr;
}
