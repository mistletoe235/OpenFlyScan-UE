...
void MultirotorPawnSimApi::setPose(const Pose& pose, bool ignore_collision)
{
    ...
    auto state = getKinematics()->getState();
    state.pose = pose;
    state.twist = Twist::zero();
    state.accelerations = Accelerations::zero();
    getKinematics()->setState(state);
    ...
    stabilizeAfterTeleport();
}

void MultirotorPawnSimApi::setKinematics(const Kinematics::State& state, bool ignore_collision)
{
    multirotor_physics_body_->lock();
    auto stabilized_state = state;
    stabilized_state.twist = Twist::zero();
    stabilized_state.accelerations = Accelerations::zero();
    getKinematics()->setState(stabilized_state);
    multirotor_physics_body_->setGrounded(false);
    multirotor_physics_body_->unlock();
    pending_pose_collisions_ = ignore_collision;
    pending_pose_status_ = PendingPoseStatus::RenderPending;
...
void MultirotorPawnSimApi::stabilizeAfterTeleport()
{
    if (vehicle_api_ == nullptr || !vehicle_api_->isApiControlEnabled())
        return;

    vehicle_api_->cancelLastTask();
    vehicle_api_->setGoalToCurrentPosition(YawMode::Zero());
}
