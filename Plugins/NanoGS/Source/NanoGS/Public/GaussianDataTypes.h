...
namespace GaussianSplattingConstants
{
	...
	/** Default color texture width (Morton-swizzled) */
	constexpr int32 ColorTextureWidth = 2048;

	/** Safe maximum 2D texture dimension for Gaussian color textures */
	constexpr int32 MaxColorTextureDimension = 16384;
...
{
	/** Pack the runtime sRGB color + linear opacity lane exactly as word 0 of
	 *  the legacy 16-byte packed splat. */
	inline uint32 PackColorOpacityToUint32(float ColorR, float ColorG, float ColorB, float Opacity)
	{
	...
	inline int64 DivideAndRoundUp64(int64 Numerator, int64 Denominator)
	{
		return (Numerator + Denominator - 1) / Denominator;
	}

	inline int32 GetColorTextureHeightForSplats(int64 InSplatCount, int32 TextureWidth)
	{
		const int64 TileSize = GaussianSplattingConstants::MortonTileSize;
		const int64 EntriesPerTile = TileSize * TileSize;
		const int64 TilesPerRow = TextureWidth / TileSize;
		const int64 NumTiles = DivideAndRoundUp64(InSplatCount, EntriesPerTile);
		const int64 TileRows = DivideAndRoundUp64(NumTiles, TilesPerRow);
		return static_cast<int32>(TileRows * TileSize);
	}

	inline int32 ChooseColorTextureWidthForSplats(int64 InSplatCount)
	{
		for (int32 Width = GaussianSplattingConstants::ColorTextureWidth;
			Width <= GaussianSplattingConstants::MaxColorTextureDimension;
			Width *= 2)
		{
			if (GetColorTextureHeightForSplats(InSplatCount, Width) <= GaussianSplattingConstants::MaxColorTextureDimension)
			{
				return Width;
			}
		}
		...
		uint32 OutWords[4])
	{
		...
		OutWords[0] = PackColorOpacityToUint32(ColorR, ColorG, ColorB, Opacity);
