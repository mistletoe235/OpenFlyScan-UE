...
public:
	 ...
	 */
	static bool DispatchCalcViewDataGlobal(
		FRHICommandListImmediate& RHICmdList,
		const FSceneView& View,
		FGaussianSplatGPUResources* GPUResources,
		const FMatrix& LocalToWorld,
	 ...
	 */
	static bool DispatchCalcDistancesGlobal(
		FRHICommandListImmediate& RHICmdList,
		FGaussianGlobalAccumulator* GlobalAccumulator,
		int32 TotalSplatCount
	);
	 ...
	 */
	static bool DispatchRadixSortGlobal(
		FRHICommandListImmediate& RHICmdList,
		FGaussianGlobalAccumulator* GlobalAccumulator,
		int32 TotalSplatCount
	);
	 ...
	 */
	static bool CompositeToSceneColor(
		FRHICommandListImmediate& RHICmdList,
		const FSceneView& View,
	...
	/**
	 * Composite display-referred Gaussian color after UE tonemapping. This path
	 * deliberately skips the sRGB-to-linear/pre-exposure conversion used by the
	 * immediate SceneColor compositor.
	 */
	static bool CompositeAfterTonemap(
		FRHICommandListImmediate& RHICmdList,
		FTextureRHIRef IntermediateTexture,
