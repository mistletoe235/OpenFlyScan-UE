...
public:
	...
	/** Exact RGBA8/opacity word split from PackedSplatBuffer for the Float32 HQ path. */
	FBufferRHIRef ColorOpacityBuffer;
	FShaderResourceViewRHIRef ColorOpacityBufferSRV;

	/** Original Float32 positions (12 bytes/splat), shared by all proxies using this asset. */
	FBufferRHIRef PositionBuffer;
	FShaderResourceViewRHIRef PositionBufferSRV;

	/** Original Float32 rotation + scale data (28 bytes/splat), shared by all proxies using this asset. */
	...
	/**
	 * Asset-local origin subtracted before float16 position packing.
	 * CalcViewData adds it back before applying LocalToWorld.
	 */
...
private:
	...
	TArray<uint8> PackedColorOpacityData;
