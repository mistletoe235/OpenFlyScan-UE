...
struct NANOGS_API FGaussianGlobalAccumulator
{
	...
	/** Stable draw ordering used by the Spark-style decoupled sort path.
	 *  Radix continues to use the primary/alt pair as scratch; only a complete
	 *  ordering is copied here and exposed to the vertex shader. */
	FBufferRHIRef GlobalPublishedSortKeysBuffer;
	FShaderResourceViewRHIRef GlobalPublishedSortKeysBufferSRV;
	bool bHasPublishedSortKeys = false;
	bool bUsePublishedSortKeysForDraw = false;
	...
	uint64 LastRejectedTotalCount = 0; ///< Suppresses per-frame log spam after an unsupported allocation request
	...
	uint32 CachedMaxRenderBudget = MAX_uint32;
	uint32 CachedRasterProfile = MAX_uint32;
	uint32 CachedFuseDepthKeys = MAX_uint32;
	uint32 CachedEnableHeuristicLOD = MAX_uint32;
	uint32 CachedUsePublishedSort = MAX_uint32;
	FMatrix LastObservedViewProjectionMatrix = FMatrix::Identity;
	bool bHasLastObservedViewProjection = false;
	uint64 InteractiveViewChangeSequence = 0;
	double LastInteractiveViewChangeSeconds = 0.0;
	bool bPublishedSortPendingExact = false;
	bool bPassTimingOneShotConsumed = false;
	 ...
	 * Returns false without changing the existing buffers when the requested
	 * capacity cannot be represented by UE's 32-bit RHI buffer sizes.
	 * Must be called from the render thread.
	 */
	bool ResizeIfNeeded(FRHICommandListBase& RHICmdList, uint64 NewTotalCount, uint32 MaxRenderBudget);
