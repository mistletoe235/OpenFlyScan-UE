...
class FGaussianSplatCalcViewDataCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_SRV(ByteAddressBuffer, ColorOpacityBuffer) // exact 4-byte RGBA8/opacity HQ stream
		SHADER_PARAMETER_SRV(ByteAddressBuffer, PositionBuffer)     // original Float32 positions (12 bytes/splat)
		SHADER_PARAMETER_SRV(ByteAddressBuffer, OtherDataBuffer)    // original Float32 quaternion + scale (28 bytes/splat)
		...
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, SortDistanceBuffer)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, SortKeyBuffer)
		...
		SHADER_PARAMETER(FVector3f, PackedPositionOrigin)  // Added to float16 packed positions before LocalToWorld
		SHADER_PARAMETER(uint32, UseFloat32Position)       // 1 = PositionBuffer, 0 = packed float16 + origin
		SHADER_PARAMETER(uint32, UseFloat32RotationScale)  // 1 = OtherDataBuffer, 0 = packed uint8 rotation/scale
		SHADER_PARAMETER(uint32, UseDedicatedColorOpacity) // 1 = standalone exact RGBA8 stream
		SHADER_PARAMETER(uint32, UseLODOpacityEncoding)    // 1 = alpha byte maps Spark LoD opacity [0,2]
		...
		SHADER_PARAMETER(uint32, RasterProfile)    // 0 = SuperSplat, 1 = Legacy LCC
...
class FPrepareIndirectArgsCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		// Runtime per-dimension dispatch limits used by wrapped indirect dispatch
...
/** Expand compact Page v1 descriptors into the active->physical index stream. */
class FNanoGSPagedBuildActiveIndexCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FNanoGSPagedBuildActiveIndexCS);
	SHADER_USE_PARAMETER_STRUCT(FNanoGSPagedBuildActiveIndexCS, FGlobalShader);

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		// uint4(ActiveBase, PhysicalBase, SplatCount, Reserved), one per active page.
		SHADER_PARAMETER_SRV(StructuredBuffer<uint4>, PageDescriptors)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, ActivePhysicalIndices)
...
/** Prepares indirect dispatch and clears the write-side frontier counter. */
class FNanoGSTreeFrontierPrepareCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FNanoGSTreeFrontierPrepareCS);
	SHADER_USE_PARAMETER_STRUCT(FNanoGSTreeFrontierPrepareCS, FGlobalShader);

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, InputCountAndOverflow)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, OutputCountAndOverflow)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, IndirectDispatchArgs)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, RequestedBlocks)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, RequestedPages)
		SHADER_PARAMETER(uint32, OutputCapacity)
	END_SHADER_PARAMETER_STRUCT()
...
/** Split-only prototype for persistent Tree v2 GPU frontier refinement. */
class FNanoGSTreeFrontierSplitCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FNanoGSTreeFrontierSplitCS);
	SHADER_USE_PARAMETER_STRUCT(FNanoGSTreeFrontierSplitCS, FGlobalShader);

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_SRV(StructuredBuffer<FTreeGPUNode>, TreeNodes)
		SHADER_PARAMETER_SRV(StructuredBuffer<FTreeGPUNode>, BlockNodes)
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, BlockResidency)
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, PageResidency)
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, InputFrontier)
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, InputPhysicalIndices)
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, InputCountAndOverflow)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, OutputFrontier)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, OutputPhysicalIndices)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, OutputCountAndOverflow)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, RequestedBlocks)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, RequestedBlockBits)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, RequestedPages)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, RequestedPageBits)
		SHADER_PARAMETER(FVector3f, ViewOriginLocal)
		SHADER_PARAMETER(float, ProjectionScale)
		SHADER_PARAMETER(float, SplitThresholdPixels)
		SHADER_PARAMETER(uint32, AllowThresholdTerminalization)
		SHADER_PARAMETER(uint32, OutputCapacity)
		SHADER_PARAMETER(uint32, BlockRequestCapacity)
		SHADER_PARAMETER(uint32, BlockAddressCapacity)
		SHADER_PARAMETER(uint32, PageRequestCapacity)
		SHADER_PARAMETER(uint32, PageAddressCapacity)
	END_SHADER_PARAMETER_STRUCT()
...
class FGaussianSplatCompositePS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, CompensateUEFilmic)
		SHADER_PARAMETER(float, ViewPreExposure)
		SHADER_PARAMETER(float, CompositeBrightness)
		SHADER_PARAMETER(FVector2f, SourceTextureSize)
		SHADER_PARAMETER(FVector2f, SourceRectMin)
		SHADER_PARAMETER(FVector2f, SourceRectSize)
		SHADER_PARAMETER(FVector2f, DestinationRectMin)
		SHADER_PARAMETER(FVector2f, DestinationRectSize)
		SHADER_PARAMETER(uint32, CompositeAfterTonemap)
	END_SHADER_PARAMETER_STRUCT()
...
class FRadixSortScatterCS : public FGlobalShader
{
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		// Keep one shader type for all supported feature levels. SM6 platforms where
		// wave operations are guaranteed use the stable wave-local rank path; SM5 and
		// runtime-dependent platforms compile the exact scalar fallback instead.
		// The stable wave histogram path is retained for further profiling, but is
		// deliberately disabled by default. On the RTX 4090/Vulkan SM6 reference
		// scene it regressed forced-recompute capture latency (59.44 ms vs 57.45 ms)
		// because LDS clears/atomics outweighed the reduced comparisons.
		const bool bUseWaveOperations = false;
		OutEnvironment.SetDefine(TEXT("RADIX_USE_WAVE_OPS"), bUseWaveOperations ? 1 : 0);
...
class FClusterCullingCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, ViewportHeightPixels)
		SHADER_PARAMETER(float, SplatScale)
