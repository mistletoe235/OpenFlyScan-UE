...
public:
	...
	/**
	 * Build a spatial cluster hierarchy without generating approximate LOD splats.
	 * The original splats and their complete SH payload are preserved; only their
	 * storage order changes to make conservative cluster culling possible.
	 */
	UFUNCTION(BlueprintCallable, Category = "Gaussian Splatting|Performance")
	bool BuildLosslessClusterHierarchy();
	...
	/** Resolve a portable project-relative source path for editor reimport operations. */
	FString GetResolvedSourceFilePath() const;

	/** Store project files relative to ProjectDir so reimport survives moving the project. */
	void SetSourceFilePathForReimport(const FString& InFilePath);
	...
	/** Number of generated approximate LOD splats (zero for a lossless hierarchy). */
	UFUNCTION(BlueprintCallable, Category = "Gaussian Splatting|Clustering")
	int32 GetGeneratedLODSplatCount() const { return static_cast<int32>(ClusterHierarchy.TotalLODSplatCount); }
...
private:
	/** Shared implementation for the lossy LOD and lossless culling hierarchies. */
	bool BuildClusterHierarchyInternal(bool bGenerateLOD);
