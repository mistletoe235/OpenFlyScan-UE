	...
	const TArray<FGaussianSplatData>& Splats)
{
	...
	for (uint32 i = Cluster.SplatStartIndex; i < EndIndex && i < (uint32)Splats.Num(); i++)
	{
		...
		// Bounds around centers alone can discard a Gaussian whose center is just
		// outside the frustum while its projected footprint is still visible. The
		// rasterizer's widest supported profile (Legacy LCC at opacity 1) reaches
		// roughly 3.33 sigma from the center. Four sigma is therefore a deliberately
		// conservative finite support and also leaves room for numeric differences.
		const float MaxSigma = FMath::Max3(
			FMath::Abs(Splat.Scale.X),
			FMath::Abs(Splat.Scale.Y),
			FMath::Abs(Splat.Scale.Z));
		const FVector3f SupportExtent(4.0f * MaxSigma);
		Cluster.ExpandBounds(Splat.Position - SupportExtent);
		Cluster.ExpandBounds(Splat.Position + SupportExtent);
	}
