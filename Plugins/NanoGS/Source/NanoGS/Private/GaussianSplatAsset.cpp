...
void UGaussianSplatAsset::PostLoad()
{
	...
	// ColorTextureBulkData is consumed once by FGaussianSplatRenderData to build
	// the packed RGBA8 word used by CalcViewData. The old runtime also recreated a
	// PF_FloatRGBA UTexture2D here, but no current shader binds that texture. Avoid
	// keeping that duplicate 8-bytes-per-splat GPU allocation alive.
	if (ColorDataSize > 0 && ColorTextureWidth > 0 && ColorTextureHeight > 0)
	{
		ColorTexture = nullptr;
		UE_LOG(LogTemp, Log, TEXT("GaussianSplatAsset::PostLoad - Color bulk data ready for packed-buffer upload (legacy runtime texture skipped)"));
	}
	else
	{
...
void UGaussianSplatAsset::CompressPositions(const TArray<FGaussianSplatData>& InSplats)
{
	...
	const int32 BytesPerSplat = 12;
	const int64 TotalBytes = static_cast<int64>(SplatCount) * BytesPerSplat;
...
{
	// Grow width when needed so very large splat sets stay within max texture height.
	ColorTextureWidth = GaussianSplattingUtils::ChooseColorTextureWidthForSplats(SplatCount);
	if (ColorTextureWidth <= 0)
	{
		UE_LOG(LogTemp, Error, TEXT("CreateColorTextureData: Failed to choose a valid texture width for %d splats"), SplatCount);
		return;
	}
	...
	const int64 NumBytes = static_cast<int64>(ColorTextureWidth) * ColorTextureHeight * sizeof(FFloat16Color);
	...
	UE_LOG(LogTemp, Log, TEXT("CreateColorTextureData: Stored %lld bytes of pixel data"), NumBytes);
}
...
void UGaussianSplatAsset::CreateColorTextureFromData()
{
	...
	const int64 NumBytes = BulkDataSize;
...
void UGaussianSplatAsset::GetPositionData(TArray<uint8>& OutData) const
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("GaussianSplatAsset: Position bulk payload (%lld bytes) exceeds monolithic TArray capacity; use Page v1 streaming."), DataSize);
		OutData.Empty();
		return;
	}
	if (DataSize > 0)
...
void UGaussianSplatAsset::GetOtherData(TArray<uint8>& OutData) const
{
	...
	if (DataSize > MAX_int32)
	{
		UE_LOG(LogTemp, Error, TEXT("GaussianSplatAsset: Rotation/scale bulk payload (%lld bytes) exceeds monolithic TArray capacity; use Page v1 streaming."), DataSize);
		OutData.Empty();
		return;
	}
	if (DataSize > 0)
...
void UGaussianSplatAsset::GetSHData(TArray<uint8>& OutData) const
{
	...
	if (DataSize > MAX_int32)
	{
		UE_LOG(LogTemp, Error, TEXT("GaussianSplatAsset: SH bulk payload (%lld bytes) exceeds monolithic TArray capacity; use Page v1 streaming."), DataSize);
		OutData.Empty();
		return;
	}
	if (DataSize > 0)
...
void UGaussianSplatAsset::GetColorTextureData(TArray<uint8>& OutData) const
{
	...
	if (DataSize > MAX_int32)
	{
		UE_LOG(LogTemp, Error, TEXT("GaussianSplatAsset: Color bulk payload (%lld bytes) exceeds monolithic TArray capacity; use Page v1 streaming."), DataSize);
		OutData.Empty();
		return;
	}
	if (DataSize > 0)
...
#if WITH_EDITOR
FString UGaussianSplatAsset::GetResolvedSourceFilePath() const
{
	...
	FString ResolvedPath = FPaths::IsRelative(SourceFilePath)
...
void UGaussianSplatAsset::SetSourceFilePathForReimport(const FString& InFilePath)
{
	...
	FString FullPath = FPaths::IsRelative(InFilePath)
		? FPaths::Combine(FPaths::ProjectDir(), InFilePath)
		: InFilePath;
	...
	SourceFilePath = FullPath.StartsWith(ProjectPrefix, ESearchCase::IgnoreCase)
		? FullPath.RightChop(ProjectPrefix.Len())
		: FullPath;
}

bool UGaussianSplatAsset::BuildNaniteClusterHierarchy()
{
	return BuildClusterHierarchyInternal(true);
}

bool UGaussianSplatAsset::BuildLosslessClusterHierarchy()
{
	return BuildClusterHierarchyInternal(false);
}

bool UGaussianSplatAsset::BuildClusterHierarchyInternal(const bool bGenerateLOD)
{
	const TCHAR* BuildMode = bGenerateLOD ? TEXT("Nanite LOD") : TEXT("lossless culling");

	// Check if source file exists
	if (SourceFilePath.IsEmpty())
	{
		UE_LOG(LogTemp, Error, TEXT("BuildClusterHierarchy (%s): Source file path is empty"), BuildMode);
		return false;
	}

	const FString ResolvedSourceFilePath = GetResolvedSourceFilePath();
	if (!FPaths::FileExists(ResolvedSourceFilePath))
	{
		UE_LOG(LogTemp, Error, TEXT("BuildClusterHierarchy (%s): Source file not found: %s"), BuildMode, *ResolvedSourceFilePath);
		return false;
	}

	FScopedSlowTask SlowTask(100.0f, FText::FromString(bGenerateLOD
		? TEXT("Building Nanite Cluster Hierarchy...")
		: TEXT("Building Lossless Culling Hierarchy...")));
	...
	if (!FPLYFileReader::ReadPLYFile(ResolvedSourceFilePath, SplatData, ErrorMessage))
	{
		UE_LOG(LogTemp, Error, TEXT("BuildClusterHierarchy (%s): Failed to read PLY file: %s"), BuildMode, *ErrorMessage);
		return false;
	}

	UE_LOG(LogTemp, Log, TEXT("BuildClusterHierarchy (%s): Read %d splats from PLY file"), BuildMode, SplatData.Num());
	...
	BuildSettings.bGenerateLOD = bGenerateLOD;
	...
	{
		UE_LOG(LogTemp, Error, TEXT("BuildClusterHierarchy (%s): Failed to build cluster hierarchy"), BuildMode);
		return false;
	}

	UE_LOG(LogTemp, Log, TEXT("BuildClusterHierarchy (%s): Built %d clusters, %d hierarchy levels, %d generated LOD splats"),
		BuildMode, NewClusterHierarchy.Clusters.Num(), NewClusterHierarchy.NumLODLevels, NewClusterHierarchy.LODSplats.Num());
	...
	if (NewClusterHierarchy.LODSplats.Num() > 0)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("BuildClusterHierarchy (%s): Unified buffer: %d original + %d LOD = %d total splats"),
			BuildMode, OriginalSplatCount, NewClusterHierarchy.LODSplats.Num(), SplatData.Num());
	...
	UE_LOG(LogTemp, Log, TEXT("BuildClusterHierarchy (%s): Complete. Total splats: %d, Clusters: %d, generated LOD splats: %d"),
		BuildMode, SplatCount, ClusterHierarchy.Clusters.Num(), ClusterHierarchy.TotalLODSplatCount);
...
void UGaussianSplatAsset::ClearNaniteClusterHierarchy()
{
	...
	if (OriginalSplatCount > 0 && OriginalSplatCount < SplatCount)
	{
		...
		const FString ResolvedSourceFilePath = GetResolvedSourceFilePath();
		if (!ResolvedSourceFilePath.IsEmpty() && FPaths::FileExists(ResolvedSourceFilePath))
		{
			TArray<FGaussianSplatData> SplatData;
			FString ErrorMessage;
