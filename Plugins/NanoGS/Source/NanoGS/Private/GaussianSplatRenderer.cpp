...
extern TAutoConsoleVariable<int32> CVarUseFloat32Position;
extern TAutoConsoleVariable<int32> CVarUseFloat32RotationScale;
extern TAutoConsoleVariable<int32> CVarRasterProfile;

// Depth handling is deliberately switchable for renderer parity diagnostics.
// SuperSplat renders without a depth buffer; UE integration normally still
// wants opaque-scene occlusion, but transparent splats should not need to
// write their center depth when the global back-to-front sort is correct.
static TAutoConsoleVariable<int32> CVarGaussianSplatDepthMode(
	TEXT("gs.DepthMode"),
	2,
	TEXT("Gaussian splat depth mode.\n")
	TEXT(" 0: SuperSplat parity (no depth test or write)\n")
	TEXT(" 1: Test against scene depth, do not write depth\n")
	TEXT(" 2: Test and write center depth (legacy default)"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarGaussianSplatCompensateUEFilmic(
	TEXT("gs.CompensateUEFilmic"),
	0,
	TEXT("Inverse-ACES compensation for display-referred Gaussian colors.\n")
	TEXT(" 0: Disabled (default; closest fixed-view parity with Spark)\n")
	TEXT(" 1: Enabled (legacy compatibility)"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarGaussianSplatApplyPreExposure(
	TEXT("gs.ApplyPreExposure"),
	1,
	TEXT("Scale Gaussian SceneColor contribution by the current view pre-exposure.\n")
	TEXT(" 0: Disabled\n")
	TEXT(" 1: Enabled (default)"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarGaussianSplatCompositeBrightness(
	TEXT("gs.CompositeBrightness"),
	1.2f,
	TEXT("Linear brightness multiplier applied during Gaussian SceneColor composition (default 1.2; two-view Spark parity)."),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarGaussianSplatAfterTonemapBrightness(
	TEXT("gs.AfterTonemapBrightness"),
	1.0f,
	TEXT("Display-referred brightness multiplier used only by gs.CompositeAfterTonemap (default 1.0)."),
	ECVF_RenderThreadSafe);

static uint32 GetGaussianRasterProfile_RenderThread()
{
	return static_cast<uint32>(FMath::Clamp(CVarRasterProfile.GetValueOnRenderThread(), 0, 1));
}

static FDepthStencilStateRHIRef GetGaussianSplatDepthStencilState()
{
	const int32 DepthMode = CVarGaussianSplatDepthMode.GetValueOnRenderThread();
	if (DepthMode <= 0)
	{
...
static uint32 ShouldUseFloat32Position(const FGaussianSplatGPUResources* GPUResources)
{
	return GPUResources &&
		GPUResources->bHasFloat32PositionData &&
		GPUResources->PositionBufferSRV.IsValid() &&
		(!GPUResources->bHasPackedGeometryData || CVarUseFloat32Position.GetValueOnRenderThread() != 0);
}

static uint32 ShouldUseFloat32RotationScale(const FGaussianSplatGPUResources* GPUResources)
{
	return GPUResources &&
		GPUResources->bHasFloat32RotationScaleData &&
		GPUResources->OtherDataBufferSRV.IsValid() &&
		(!GPUResources->bHasPackedGeometryData || CVarUseFloat32RotationScale.GetValueOnRenderThread() != 0);
}
	...
	FGaussianGlobalAccumulator* Accumulator)
{
	...
	PSParameters.RasterProfile = GetGaussianRasterProfile_RenderThread();
...
// Compute world-to-local matrix for SH evaluation.
// SH coefficients are evaluated in the same local basis used by the imported splat data.
	...
	bool bUseLODRendering)
{
	...
	Parameters.SortDistanceBuffer = GPUResources->SortDistanceBufferUAV;
	Parameters.SortKeyBuffer = GPUResources->SortKeysBufferUAV;
	...
	Parameters.PackedPositionOrigin = GPUResources->PackedPositionOrigin;
	Parameters.UseFloat32Position = ShouldUseFloat32Position(GPUResources);
	Parameters.UseFloat32RotationScale = ShouldUseFloat32RotationScale(GPUResources);
	Parameters.UseDedicatedColorOpacity = GPUResources->bUsesDedicatedColorOpacityBuffer ? 1u : 0u;
	Parameters.UseLODOpacityEncoding = GPUResources->bUsesLODOpacityEncoding ? 1u : 0u;
	...
	Parameters.RasterProfile = GetGaussianRasterProfile_RenderThread();
	...
	const int32 ThreadGroupSize = 256;
	const FIntVector GroupCount = FComputeShaderUtils::GetGroupCountWrapped(
		SplatCount,
		ThreadGroupSize);
	...
	// ViewData remains in UAV state until draw; radix sort consumes the fused keys.
}
	...
	float OpacityScale)
{
	...
	VSParameters.RasterProfile = GetGaussianRasterProfile_RenderThread();
	...
	PSParameters.OpacityScale = OpacityScale;
	...
	bool bUseLODRendering)
{
	...
	// Reset visible splat count on GPU. LockBuffer/UnlockBuffer from within a render pass
	// trips Vulkan validation/asserts on UE5.5.
	...
	RHICmdList.ClearUAVUint(GPUResources->VisibleSplatCountBufferUAV, FUintVector4(0u, 0u, 0u, 0u));
	...
	FGaussianSplatGPUResources* GPUResources)
{
	...
	Parameters.MaxDispatchThreadGroupsPerDimension = GRHIMaxDispatchThreadGroupsPerDimension;
...
bool FGaussianSplatRenderer::DispatchCalcViewDataGlobal(
	FRHICommandListImmediate& RHICmdList,
	const FSceneView& View,
	FGaussianSplatGPUResources* GPUResources,
	const FMatrix& LocalToWorld,
	int32 SplatCount,
	uint32 MaxSplatCountToProcess,
	int32 SHOrder,
	float OpacityScale,
	float SplatScale,
	bool bUseLODRendering,
	...
	FGaussianGlobalAccumulator* GlobalAccumulator)
{
	...
	if (GPUResources == nullptr || GlobalAccumulator == nullptr ||
		SplatCount <= 0 || MaxSplatCountToProcess == 0)
	{
		return false;
	}

	// The render budget can end part-way through the final proxy. Preserve the
	// full source count for OriginalSplatCount, but guard and dispatch only the
	// prefix that fits in the global output buffers.
	const uint32 DispatchSplatCount = FMath::Min(static_cast<uint32>(SplatCount), MaxSplatCountToProcess);
	...
	Parameters.SortDistanceBuffer = GlobalAccumulator->GlobalSortDistanceBufferUAV;
	Parameters.SortKeyBuffer = GlobalAccumulator->GlobalSortKeysBufferUAV;
	...
	else
	{
		Parameters.UseClusterCulling = 0;
		Parameters.UseLODRendering = 0;
		// Page residency's active list contains stable physical pool indices,
		// which can be greater than ActiveCount when slots contain gaps.
		Parameters.OriginalSplatCount = GPUResources->bHasPrecompactedActiveList
			? GPUResources->GetSourceSplatCapacity()
			: static_cast<uint32>(SplatCount);
	}

	Parameters.CompactedSplatIndices = GPUResources->CompactedSplatIndicesBufferSRV;
	Parameters.UseCompaction = GPUResources->bHasPrecompactedActiveList ? 1u : 0u;
	Parameters.VisibleSplatCount = GPUResources->bHasPrecompactedActiveList ? DispatchSplatCount : 0u;
	...
	Parameters.SplatCount = DispatchSplatCount;
	// Use effective SH order: min(requested order, available data)
	int32 EffectiveSHOrder = FMath::Min(SHOrder, GPUResources->GetSHBands());
	Parameters.SHOrder = EffectiveSHOrder;
	// SH buffer includes DC + higher-order coefficients: band1=4, band2=9, band3=16
	...
	// Direct global output path. A paged source uses a pre-authored active list,
	// but still writes at the CPU-computed global base and shares the same sort/draw.
	Parameters.GlobalBaseOffsetsBuffer = GPUResources->CompactedSplatIndicesBufferSRV;  // dummy
	Parameters.ProxyIndex = 0;
	Parameters.UseGlobalCompactionPath = 0;
	Parameters.MaxRenderBudget = 0;
	...
	const int32 ThreadGroupSize = 256;
	const FIntVector GroupCount = FComputeShaderUtils::GetGroupCountWrapped(
		static_cast<int32>(DispatchSplatCount),
		ThreadGroupSize);
...
bool FGaussianSplatRenderer::DispatchCalcDistancesGlobal(
	FRHICommandListImmediate& RHICmdList,
	FGaussianGlobalAccumulator* GlobalAccumulator,
	int32 TotalSplatCount)
{
	SCOPED_DRAW_EVENT(RHICmdList, GaussianSplatCalcDistancesGlobal);
	if (GlobalAccumulator == nullptr || TotalSplatCount <= 0)
	{
		return false;
	}
	...
	const FIntVector GroupCount = FComputeShaderUtils::GetGroupCountWrapped(
		TotalSplatCount,
		static_cast<int32>(ThreadGroupSize));
...
bool FGaussianSplatRenderer::DispatchRadixSortGlobal(
	FRHICommandListImmediate& RHICmdList,
	FGaussianGlobalAccumulator* GlobalAccumulator,
	int32 TotalSplatCount)
{
...
bool FGaussianSplatRenderer::DrawSplatsGlobal(
	FRHICommandListImmediate& RHICmdList,
	const FSceneView& View,
	FGaussianGlobalAccumulator* GlobalAccumulator,
	...
	float OpacityScale)
{
	...
	if (GlobalAccumulator == nullptr || TotalSplatCount <= 0 || !IndexBuffer.IsValid())
	{
		return false;
	}
	...
	VSParameters.SortKeysBuffer =
		GlobalAccumulator->bUsePublishedSortKeysForDraw &&
		GlobalAccumulator->bHasPublishedSortKeys
			? GlobalAccumulator->GlobalPublishedSortKeysBufferSRV
			: GlobalAccumulator->GlobalSortKeysBufferSRV;
	...
	bool bUseLODRendering)
{
	...
	// Step 2: Run cluster culling with LOD selection
	{
		...
		CullingParams.ViewportHeightPixels = static_cast<float>(FMath::Max(View.UnscaledViewRect.Height(), 1));
		CullingParams.SplatScale = FMath::Abs(SplatScale);
...
bool FGaussianSplatRenderer::CompositeToSceneColor(
	FRHICommandListImmediate& RHICmdList,
	const FSceneView& View,
	FTextureRHIRef IntermediateTexture)
{
	...
	PSParameters.CompensateUEFilmic = CVarGaussianSplatCompensateUEFilmic.GetValueOnRenderThread() != 0 ? 1u : 0u;
	PSParameters.ViewPreExposure = CVarGaussianSplatApplyPreExposure.GetValueOnRenderThread() != 0
		? FMath::Max(ViewInfo.PreExposure, UE_SMALL_NUMBER)
		: 1.0f;
	PSParameters.CompositeBrightness = FMath::Max(
		CVarGaussianSplatCompositeBrightness.GetValueOnRenderThread(), 0.0f);
	PSParameters.SourceTextureSize = FVector2f(ViewRect.Width(), ViewRect.Height());
	PSParameters.SourceRectMin = FVector2f(ViewRect.Min.X, ViewRect.Min.Y);
	PSParameters.SourceRectSize = FVector2f(ViewRect.Width(), ViewRect.Height());
	PSParameters.DestinationRectMin = FVector2f(ViewRect.Min.X, ViewRect.Min.Y);
	PSParameters.DestinationRectSize = FVector2f(ViewRect.Width(), ViewRect.Height());
	PSParameters.CompositeAfterTonemap = 0u;
	SetShaderParameters(RHICmdList, PixelShader, PixelShader.GetPixelShader(), PSParameters);
...
bool FGaussianSplatRenderer::CompositeAfterTonemap(
	FRHICommandListImmediate& RHICmdList,
	FTextureRHIRef IntermediateTexture,
...
{
	SCOPED_DRAW_EVENT(RHICmdList, GaussianSplatCompositeAfterTonemap);
	if (!IntermediateTexture.IsValid() || SourceRect.IsEmpty() || DestinationRect.IsEmpty())
	{
		return false;
	}
	...
	GraphicsPSOInit.DepthStencilState = TStaticDepthStencilState<false, CF_Always>::GetRHI();
	GraphicsPSOInit.BlendState = TStaticBlendState<
		CW_RGB, BO_Add, BF_One, BF_InverseSourceAlpha,
	...
	RHICmdList.SetViewport(
		DestinationRect.Min.X, DestinationRect.Min.Y, 0.0f,
		DestinationRect.Max.X, DestinationRect.Max.Y, 1.0f);

	FGaussianSplatCompositePS::FParameters PSParameters;
	PSParameters.IntermediateTexture = IntermediateTexture;
	PSParameters.IntermediateSampler =
		TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
	PSParameters.CompensateUEFilmic = 0u;
	PSParameters.ViewPreExposure = 1.0f;
	PSParameters.CompositeBrightness = FMath::Max(
		CVarGaussianSplatAfterTonemapBrightness.GetValueOnRenderThread(), 0.0f);
	PSParameters.SourceTextureSize = FVector2f(SourceTextureExtent.X, SourceTextureExtent.Y);
	PSParameters.SourceRectMin = FVector2f(SourceRect.Min.X, SourceRect.Min.Y);
	PSParameters.SourceRectSize = FVector2f(SourceRect.Width(), SourceRect.Height());
	PSParameters.DestinationRectMin = FVector2f(DestinationRect.Min.X, DestinationRect.Min.Y);
	PSParameters.DestinationRectSize = FVector2f(DestinationRect.Width(), DestinationRect.Height());
	PSParameters.CompositeAfterTonemap = 1u;
	SetShaderParameters(RHICmdList, PixelShader, PixelShader.GetPixelShader(), PSParameters);
